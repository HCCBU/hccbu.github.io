---
layout: post
title: [Posters and workshop paper at IEEE VIS 2018]
categories: news
tags:
- InfoVis
- conference
- news
- Berlin
related:
list: true
type: home
conference_map:
  location_id: berlin-germany
  city: Berlin
  country: Germany
  lat: 52.52
  lng: 13.405
  conference: IEEE VIS 2018
  year: 2018
  thumbnail: /assets/images/Vis2018.jpeg
---

We presented the following two posters and a workshop paper, in the [_International Conference on Visualization (IEEE VIS 2018)_](http://ieeevis.org/year/2018/welcome), held in Berlin, Germany, on the 21th - 26th of October, 2018. 

{% bibliography -f posters --query @*[key=Butcher-et-al-Poster-VIS2018] %}

{% bibliography -f posters --query @*[key=Jackson-et-al-Poster-VIS2018] %}

{% bibliography -f workshops --query @*[key=Varghese-et-al-VISREG-2018] %}

